  ...
  return (
    <PageShell>
      <div className="flex w-full items-start justify-between gap-4">
        <PageTitle
          title="UPLOAD"
          subtitle="Upload your book to explore its emotional DNA."
        />
        <div className="relative">
          <button
            onClick={() => setShowInfo(!showInfo)}
            aria-label="Privacy and copyright"
            className="-mr-2 p-2 rounded-full hover:bg-stone-100 transition-colors"
          >
            <Info className="w-6 h-6 text-stone-600 hover:text-stone-900" />
          </button>

          {showInfo && (
            <div className="absolute right-0 mt-2 w-[min(20rem,calc(100vw-5rem))] bg-white border border-stone-300 rounded-lg shadow-lg p-4 z-50">
        ...
        >
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2">
              ...
              <p className="text-[#4D8937] font-medium">Upload successful!</p>
            </div>
          ) : (
